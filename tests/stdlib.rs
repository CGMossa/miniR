...
fn options_get_set_roundtrip() {
    ...
    assert_eq!(val.as_vector().and_then(|v| v.as_double_scalar()), Some(42.0));
}
...
fn get_option_with_default() {
    ...
    let val = r
        ...
        .unwrap()
        .value;
    assert_eq!(val.as_vector().and_then(|v| v.as_double_scalar()), Some(99.0));
}
...
fn debug_stubs_do_not_error() {
    let mut r = Session::new();
    r.eval_source("f <- function() 1; debug(f); undebug(f)").unwrap();
