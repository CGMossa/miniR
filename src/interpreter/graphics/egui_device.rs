                ...
                for (idx, item) in self.state.items.iter().enumerate() {
                    ...
                    match item {
                        ...
                        PlotItem::BoxPlot {
                            ...
                            for (j, (pos, spread)) in
                                positions.iter().zip(spreads.iter()).enumerate()
                            {
...
/// Launch a blocking egui window displaying the current plot.
///
/// Converts `PlotItem`s to egui_plot items and shows the window.
...
pub fn show_plot_window(state: &PlotState) -> Result<(), String> {
    ...
    eframe::run_simple_native(&title, native_options, move |ctx, frame| {
        let mut app = PlotApp {
            state: owned_state.clone(),
        };
